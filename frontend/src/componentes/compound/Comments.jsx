        ...
        >
          <Input name='content' kind='textarea' placeholder='Escribe tu comentario aquí' label={null} minRows={1} />
            <div className='flex flex-row gap-2 w-full'>
              <Input name='rating' kind='rating' placeholder='Calificación (0-10)' label={null} className='w-2/3' />
              <Button type='submit' kind='primary' size='sm' isLoading={addCommentMutation.isPending} className='w-1/3'>{'Enviar'}</Button>
    ...
    <Container shadow='none' className='w-full'>
      <Container.Body className='flex flex-col justify-between w-full items-center'>
        <div className="flex flex-row justify-between w-full items-center">
          <UserCard kind='comment' user={comment.creator} description={ratingStars(comment.rating)}/>
        ...
        <Text className='whitespace-pre-wrap break-words text-start pl-2 w-full h-full'>{comment.content}</Text>
      </Container.Body>
    </Container>
  );
};
