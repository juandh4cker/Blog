...
import bcrypt, json, jwt, logging, os, re
...
#Token seguro, no en localstoragge
...
def decode_jwt(token):
    if not token:
        return {"verify": False, "error": "Token requerido"}, 200

    try:
        decoded_token = jwt.decode(token, app.config["JWT_SECRET_KEY"], algorithms=["HS256"])
        return {"verify": True, "user": decoded_token}
    except jwt.ExpiredSignatureError:
        return {"verify": False, "error": "Token expirado"}, 200
    except jwt.InvalidTokenError:
        return {"verify": False, "error": "Token inválido"}, 200
...
@app.route('/api/mongoposts', methods=['GET'])
def get_blogsss():
    try:
        MONGO_URI = os.getenv("MONGO_URI", "mongodb://admin:password@mongodb:27017/worldblog?authSource=admin")
        ...
        return jsonify(posts)  # Excluye _id y solo devuelve "comments"
    
    except Exception as e:
        return jsonify({"error": str(e)}), 500
    
@app.route('/api/mongousers', methods=['GET'])
def get_userssss():
    try:
        MONGO_URI = os.getenv("MONGO_URI", "mongodb://admin:password@mongodb:27017/worldblog?authSource=admin")
        client = MongoClient(MONGO_URI)
        ...
        return jsonify(users)  # Excluye _id y solo devuelve "comments"
